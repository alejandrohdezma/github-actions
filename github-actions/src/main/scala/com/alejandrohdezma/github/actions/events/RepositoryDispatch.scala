...
/** You can use the GitHub API to trigger a webhook event called repository_dispatch when you want to trigger a workflow
  * for activity that happens outside of GitHub.
  *
  ...
  *
  * @param types
  *   the event_type under which the repository_dispatch should trigger this workflow
  *
  * @see
  *   See more information about the
  *   [[https://developer.github.com/v3/repos/#create-a-repository-dispatch-eventRESTAPI]].
...
final case class RepositoryDispatch(types: List[NonEmptyString]) extends Event {
  ...
  def types(newTypes: NonEmptyString*): RepositoryDispatch =
