 ...
 */
final case class Defaults(shell: Option[Shell] = None, workingDirectory: Option[NonEmptyString] = None)

object Defaults {

  implicit lazy val DefaultsEncoder: Encoder[Defaults] = {
    case d: Defaults if d.shell.nonEmpty || d.workingDirectory.nonEmpty =>
      Json.obj("run" := Json.obj("shell" := d.shell, "working-directory" := d.workingDirectory))
    case _ => Json.Null
  }
