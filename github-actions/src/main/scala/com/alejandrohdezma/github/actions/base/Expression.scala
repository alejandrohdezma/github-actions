...
sealed trait Expression {
  ...
  private def showR(parentheses: Boolean = false /* scalafix:ok */ ): String = this match {
    ...
    case Expression.ExtendableExpression(value)                          => value.value
    ...
    case Expression.Operator.Not(Expression.ExtendableExpression(value)) => s"!${value.value}"
...
object Expression {
  ...
  final case class ExtendableExpression private (private val value: NotEmptyString) extends Expression with Dynamic {

    def selectDynamic(name: String): ExtendableExpression = ExtendableExpression(NotEmptyString.unsafe(s"$value.$name"))
