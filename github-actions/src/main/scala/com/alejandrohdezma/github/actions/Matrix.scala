...
/** A build matrix is a set of different configurations of the virtual environment. For example you might run a job
  * against more than one supported version of a language, operating system, or tool. Each configuration is a copy of
  * the job that runs and reports a status.
  *
...
final case class Matrix(
    ...
    include: List[Map[NonEmptyString, NonEmptyString]] = Nil,
    exclude: List[Map[NonEmptyString, NonEmptyString]] = Nil
)
...
object Matrix {
  ...
  final case class Configuration(
      key: NonEmptyString,
      values: NonEmptyList[NonEmptyString]
  )

  /** Allows converting a [[Matrix]] value into [[yaml.Yaml yaml]]. */
  implicit val MatrixEncoder: Encoder[Matrix] = matrix => {
    val jsonExclude = matrix.exclude.filter(_.nonEmpty) match {
      case Nil => Yaml.obj()
      case list =>
        Yaml.obj("exclude" := list.map(_.map(t => t._1.value -> t._2)))
    }

    val jsonInclude = matrix.include.filter(_.nonEmpty) match {
      case Nil => Yaml.obj()
      case list =>
        Yaml.obj("include" := list.map(_.map(t => t._1.value -> t._2)))
    }

    jsonExclude
      .merge(jsonInclude)
      .merge(matrix.value.map(c => c.key.value -> c.values).value.toMap.asYaml)
  }
