...
/** Input parameters allow you to specify data that the action expects to use during runtime. GitHub stores input
  ...
  *
  * @param name
  *   a string identifier to associate with the input. The value of <input_id> is a map of the input's metadata. The
  *   <input_id> must be a unique identifier within the inputs object. The <input_id> must start with a letter or _ and
  ...
  * @param default
  *   a string representing the default value. The default value is used when an input parameter isn't specified in a
  *   workflow file
  */
final case class Input(
    name: Id,
    description: Option[NonEmptyString] = None,
    deprecationMessage: Option[NonEmptyString] = None,
    required: Boolean = false,
    default: Option[NonEmptyString] = None
) {

  def description(newDescription: NonEmptyString): Input =
    copy(description = Some(newDescription))

  def deprecationMessage(message: NonEmptyString): Input =
    copy(deprecationMessage = Some(message))

  def isRequired(): Input = copy(required = true)

  def default(newDefault: NonEmptyString): Input =
    copy(default = Some(newDefault))
...
object Input {
  ...
  /** Allows converting a [[Input]] value into [[yaml.Yaml yaml]]. */
