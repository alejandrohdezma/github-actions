...
/** Runs your workflow anytime a comment on a pull request's unified diff is modified, which triggers the
  ...
  *
  * @see
  *   See more information about the [[https://developer.github.com/v3/pulls/commentsRESTAPI]].
...
final case class PullRequestReviewComment(
    types: List[PullRequestReviewComment.Types]
) extends Event {

  def onCreated() =
    copy(types = types :+ PullRequestReviewComment.Types.Created)

  def onEdited() = copy(types = types :+ PullRequestReviewComment.Types.Edited)

  def onDeleted() =
    copy(types = types :+ PullRequestReviewComment.Types.Deleted)
