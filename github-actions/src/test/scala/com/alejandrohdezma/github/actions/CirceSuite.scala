...
class CirceSuite extends munit.FunSuite {
  ...
  test("Create ci.yml workflow") {
      ...
      workflow
          ...
          workflowDispatch
            .input("repository") {
              _.description("The repository to use")
                .deprecationMessage(
                  "This input is deprecated, use repositories instead"
                )
                .isRequired()
                .default(env.GITHUB_REPOSITORY)
            }
            .input("repositories") {
              _.description("Comma-separated list of repositories")
        ...
        .job("scala_steward") {
          _.name("Run `sbt fix generateCiFiles` in Scala Steward PRs")
            ...
            .continueOnError()
            .runsIf(
              is[pullRequest] && (github.event.pull_request.head.repo.full_name === github.repository) &&
                github.event.pull_request.user.login === "alejandrohdezma-steward[bot]"
            )
            .output("changes_detected", steps.push.outputs.changes_detected)
            .container(
              image("ubuntu:latest")
                .credentials(
                  username = "alejandrohdezma",
            ...
            .service(
              "redis",
              container = image("redis:latest").exposePort(4004)
            )
            .strategy(
              matrix
        ...
        .job("test-2") {
          _.runsOn(
            selfHosted.machine(linux).architecture(x64).labels("miau", "meow")
          )
    ...
    val ci = workflowFile("ci") {
      workflow
        ...
        .job("update_release_draft") {
          _.name(
            "Drafts/updates the next repository release and update PR labels"
          )
            .runsIf(
              is[push] || is[pullRequest] && (github.event.pull_request.head.repo.full_name === github.repository)
            )
            .steps(
              ...
              uses("release-drafter/release-drafter@v5")
                .env("GITHUB_TOKEN", steps.installation_token.outputs.token)
            )
        }
        .job("update-prs") {
          _.name("Update outdated PRs to latest main")
            ...
            .steps(
              ...
              uses("adRise/update-pr-branch@v0.4.0")
                ...
                .withInput("token", steps.installation_token.outputs.token)
                .withInput("base", "main")
        ...
        .job("scala-steward") {
          _.name(
            "Process Scala Steward PRs: auto-approves, enable auto-merge..."
          )
            .runsIf(
              is[pullRequest] && github.event.pull_request.head.repo.full_name === github.repository &&
                github.event.pull_request.user.login === "alejandrohdezma-steward[bot]"
            ...
            .steps(
              ...
              uses("actions/checkout@v2.3.4")
                ...
                .withInput("token", steps.installation_token.outputs.token)
                .withInput("ref", github.event.pull_request.head.ref),
              uses("coursier/cache-action@v6")
              ...
              uses("kentaro-m/auto-assign-action@v1.1.2")
                .runsIf(steps.check_files.outputs.files_exists === "true")
                .name("Add Pull Request Reviewer")
                .withInput(
                  "repo-token",
                  steps.installation_token.outputs.token
                ),
              ...
              uses("stefanzweifel/git-auto-commit-action@v4.9.2")
                ...
                .withInput(
                  "commit_message",
                  "Regenerate files with `sbt generateCiFiles fix`"
                )
            )
        }
        .job("test") {
          _.needs("scala-steward")
            .runsIf(
              always && is[pullRequest] && needs.`scala-steward`.outputs.changes_detected =!= "true"
            )
            .name(s"""Run "sbt ci-test" on JDK ${matrix.jdk}""")
            .strategy(
              matrix
    ...
    val newWorkflowFiles =
      workflowFiles.modify(_.eachWhere(_.fileName.is("test")).workflow) {
        _.disableDefaults()
          ...
          .modify(_.jobs.value.eachWhere(_.id.is("scala_steward"))) {
            _.disableEnvironment().modify(
              _.steps.value.each.eachRight.id.at.value
            )(_.toUpperCase())
          }
      }

    val yamlFiles =
      newWorkflowFiles.map(file => file.fileName -> file.workflow.asYaml)
    ...
    newYamlFiles.foreach { case (name, yaml) =>
      java.nio.file.Files.write(
        java.nio.file.Paths.get(
          s"${sys.props("user.dir")}/.github/workflows/${name.value}.yml"
        ), // scalafix:ok
