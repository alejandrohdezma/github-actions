 ...
 */
final case class RepositoryDispatch(types: List[NotEmptyString]) extends Event {

  def types(newTypes: NotEmptyString*): RepositoryDispatch = copy(types = types ++ newTypes)
