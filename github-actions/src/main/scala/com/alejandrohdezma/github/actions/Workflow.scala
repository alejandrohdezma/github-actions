...
/** @param name
  *   the name of your workflow. GitHub displays the names of your workflows on your repository's actions page. If you
  *   omit this field, GitHub sets the name to the workflow's filename.
  * @param on
  *   the name of the GitHub event that triggers the workflow. You can provide a single event string, array of events,
  *   array of event types, or an event configuration map that schedules a workflow or restricts the execution of a
  *   workflow to specific files, tags, or branch changes. For a list of available events, see
  ...
  * @param jobs
  *   the list of jobs that conform a workflow
  *
...
final case class Workflow(
  ...
  def name(name: NonEmptyString): Workflow = copy(name = Some(name))

  def env(key: NonEmptyString, value: NonEmptyString): Workflow =
    copy(env = env + ((key, value)))

  def env(key: NonEmptyString, value: Expression): Workflow =
    env(key, value.show())

  def disableDefaults(): Workflow = copy(defaults = None)

  def defaults(shell: Shell, workingDirectory: NonEmptyString): Workflow =
  ...
  def defaults(workingDirectory: NonEmptyString): Workflow =
    copy(defaults =
      defaults
  ...
  def on(first: Event, rest: Event*): Workflow =
    copy(on = on ::: NonEmptyList.of(first, rest: _*))

  def job(id: Id)(builder: Job.Builder => Job): Workflow =
    copy(jobs = jobs :+ builder(Job.Builder(id)))
...
object Workflow {
  ...
  final case class Builder1(
    ...
    def env(key: NonEmptyString, value: Expression): Builder1 =
      env(key, value.show())
    ...
    def on(first: Event, rest: Event*): Builder2 =
      Builder2(
        ...
        env = env,
        defaults = defaults,
        on = NonEmptyList.of(first, rest: _*)
      )
  ...
  final case class Builder2(
    ...
    def env(key: NonEmptyString, value: Expression): Builder2 =
      env(key, value.show())
    ...
    def job(id: Id)(builder: Job.Builder => Job): Workflow =
      Workflow(
        ...
        env = env,
        defaults = defaults,
        jobs = NonEmptyList.of(builder(Job.Builder(id)))
      )
  ...
  implicit lazy val WorkflowEncoder: Encoder[Workflow] = workflow =>
    Yaml.obj(
      ...
      "env" := (if (workflow.env.isEmpty) Yaml.Null
                else workflow.env.map(t => t._1.value -> t._2).asYaml),
