...
object Strategy {
  ...
  final case class FromMatrix(
    ...
    def config(key: NotEmptyString)(first: NotEmptyString, rest: NotEmptyString*) =
      copy(value = value.copy(value = value.value :+ Matrix.Configuration(key, NotEmptyList.of(first, rest: _*))))

    def include(configurations: (NotEmptyString, NotEmptyString)*): FromMatrix =
      copy(value = value.copy(include = value.include.::(Map(configurations: _*))))

    def exclude(configurations: (NotEmptyString, NotEmptyString)*): FromMatrix =
