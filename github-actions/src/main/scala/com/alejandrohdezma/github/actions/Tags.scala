...
object Tags {
  ...
  final case class Matching(matches: NonEmptyList[NonEmptyString]) extends Tags
  def matching(first: NonEmptyString, rest: NonEmptyString*) = Matching(NonEmptyList.of(first, rest: _*))

  final case class Ignore(matches: NonEmptyList[NonEmptyString]) extends Tags
  ...
  implicit val TagsEncoder: Encoder[Tags] = {
    case Tags.Matching(matches) => Json.obj("tags" := matches)
    case Tags.Ignore(matches)   => Json.obj("tags-ignore" := matches)
  }
