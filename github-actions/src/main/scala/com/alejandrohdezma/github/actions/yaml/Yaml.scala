...
/** Represents a valid YAML value.
  *
  * Available implementations are:
  *
  * - [[Yaml.Object]]: For top level objects.
  * - [[Yaml.Array]]: For lists of [[Yaml]] values.
  * - [[Yaml.Scalar]]: For scalar values (mainly strings).
  * - [[Yaml.Bool]]: For boolean values.
  * - [[Yaml.Number]]: For numbers.
  * - [[Yaml.Null]]: For missing values.
  */
sealed trait Yaml {

  /** Merges the current [[Yaml]] value with a provided one.
    *
    * Only works if the two values are [[Yaml.Object]] or [[Yaml.Array]] (or [[Yaml.Null]]).
    *
    * Otherwise it will just return the supplied value.
    */
  def merge(that: Yaml): Yaml = (this, that) match {
    case (Yaml.Object(thisValue), Yaml.Object(thatValue)) =>
      Yaml.Object(thisValue ++ thatValue)
    case (Yaml.Object(thisValue), Yaml.Null) => Yaml.Object(thisValue)
    case (Yaml.Null, Yaml.Object(thatValue)) => Yaml.Object(thatValue)
    case (Yaml.Array(thisValue), Yaml.Array(thatValue)) =>
      Yaml.Array(thisValue ++ thatValue)
  ...
  /** Pretty-prints this [[Yaml]] as a valid YAML string. */
  def prettyPrint(): String = print("")

  private[yaml] def print(indent: String): String =
    this match {
      case Yaml.Null                                                => ""
      ...
      case Yaml.Object(map) =>
        map
          ...
          .map {
            ...
            case (key, array: Yaml.Array) if array.isSmallScalarArray =>
              s"$indent$key: ${array.print("")}"
            case (key, array: Yaml.Array) =>
              s"$indent$key:\n${array.print(indent + "  ")}"
...
/** Contains the available [[Yaml]] classes as well as smart-constructors for creating [[Yaml]] values. */
object Yaml {

  /** Convenient value for indicating an empty [[Yaml.Object]]. */
  ...
  /** Represents a YAML object. Containing a list of fields (string/YAML value). */
  final case class Object(value: Map[String, Yaml]) extends Yaml

  /** Represents a YAML array. Contianing a list of YAML values. */
  final case class Array(value: List[Yaml]) extends Yaml {

    /** Returns `true` if this array is only formed by five or less [[Scalar]] (strings). */
  ...
  final case class Scalar(val value: String) extends Yaml {
    ...
    /** Returns the value wrapped in single quotes if it contains any of the YAML special characters; otherwise returns
      * the raw value.
      */
    lazy val formatted =
      if (
        value.contains("*") || value
          .startsWith("[") || value.startsWith("!") || value.contains(": ")
      ) s"'$value'"
      else value
  ...
  /** Represents a YAML number value. */
  final case class Number(value: Int) extends Yaml

  /** Represents a YAML null value. Meaning that the value is missing. */
