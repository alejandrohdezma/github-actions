...
object Paths {
  ...
  final case class Matching(matches: NonEmptyList[NonEmptyString]) extends Paths
  def matching(first: NonEmptyString, rest: NonEmptyString*) = Matching(NonEmptyList.of(first, rest: _*))

  final case class Ignore(matches: NonEmptyList[NonEmptyString]) extends Paths
  ...
  implicit val PathsEncoder: Encoder[Paths] = {
    case Paths.Matching(matches) => Json.obj("paths" := matches)
    case Paths.Ignore(matches)   => Json.obj("paths-ignore" := matches)
  }
