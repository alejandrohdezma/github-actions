...
/** Selects an action to run as part of a step in your job. An action is a reusable unit of code. You can use an action
  ...
  *   the action to run
  * @param with
  *   a map of the input parameters defined by the action. Each input parameter is a key/value pair. Input parameters
  *   are set as environment variables. The variable is prefixed with INPUT_ and converted to upper case
  * @param id
...
final case class Uses(
    ...
    `with`: Map[NonEmptyString, NonEmptyString] = ListMap(),
  ...
  def withInput(key: NonEmptyString, value: Expression): Uses =
    withInput(key, value.show())

  def withInput(key: NonEmptyString, value: NonEmptyString): Uses =
    copy(`with` = `with` + ((key, value)))

  def id(id: NonEmptyString): Uses =
    copy(id = Some(id))

  def runsIf(expression: Expression): Uses =
    copy(`if` = Some(expression))

  def name(name: NonEmptyString): Uses = copy(name = Some(name))

  def env(key: NonEmptyString, value: NonEmptyString): Uses =
    copy(env = env + ((key, value)))

  def env(key: NonEmptyString, value: Expression): Uses = env(key, value.show())

  def continueOnError(): Uses = copy(`continue-on-error` = Some(Left(true)))

  def doNotContinueOnError(): Uses =
    copy(`continue-on-error` = Some(Left(false)))

  def continueOnErrorIf(expression: Expression): Uses =
    copy(`continue-on-error` = Some(Right(expression)))

  def timeoutMinutes(timeoutMinutes: Int): Uses =
...
object Uses {
  ...
  /** Allows converting a [[Uses]] value into [[yaml.Yaml yaml]]. */
  implicit val UsesEncoder: Encoder[Uses] = step =>
    Yaml.obj(
      ...
      "id"   := step.id,
      "uses" := step.action,
      "with" := (if (step.`with`.isEmpty) Yaml.Null
                 else step.`with`.map(t => t._1.value -> t._2).asYaml),
