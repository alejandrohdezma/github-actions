...
final case class WorkflowDispatch(inputs: List[Input]) extends Event {
  ...
  def input(name: Id)(builder: Input => Input): WorkflowDispatch =
    copy(inputs = inputs :+ builder(Input(name)))

}

object WorkflowDispatch {

  implicit val WorkflowDispatchEncoder: Encoder[WorkflowDispatch] =
    _.inputs.map(_.asYaml).reduce((a, b) => b.merge(a))
