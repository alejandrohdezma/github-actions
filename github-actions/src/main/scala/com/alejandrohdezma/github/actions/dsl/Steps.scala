...
trait Steps {
  ...
  /** A job contains a sequence of tasks called steps. Steps can run commands, run setup tasks, or run an action in your
    ...
    *
    * @param id a unique identifier for the step. You can use the id to reference the step in contexts. For more information,
    ...
    * @param env sets environment variables for steps to use in the virtual environment. You can also set environment variables
    *   for the entire workflow or a job
    * @param continueOnError prevents a job from failing when a step fails. Set to true to allow a job to pass when this step fails
  ...
  implicit def Run2Either(run: Run): Either[Uses, Run] =
    Right(run) /* scalafix:ok DisableSyntax.implicitConversion */

  def uses(action: NonEmptyString): Uses = Uses(action)

  implicit def Uses2Either(run: Uses): Either[Uses, Run] =
    Left(run) /* scalafix:ok DisableSyntax.implicitConversion */
