...
trait Events {
  ...
  def matching(first: NonEmptyString, rest: NonEmptyString*): Matching =
    Matching(NonEmptyList.of(first, rest: _*))

  def ignore(first: NonEmptyString, rest: NonEmptyString*): Ignoring =
    Ignoring(NonEmptyList.of(first, rest: _*))
