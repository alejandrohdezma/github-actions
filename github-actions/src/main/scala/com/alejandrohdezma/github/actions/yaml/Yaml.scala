    ...
    this match {
      ...
      case Yaml.Object(map) =>
        map
          ...
          .mkString("\n")
      case array @ Yaml.Array(list) if array.isSmallScalarArray =>
        list.map(_.print(s"")).mkString("[", ", ", "]")
      case Yaml.Array(list) =>
        list
          .map(_.print(s"$indent  ").replaceFirst(s"$indent  ", s"$indent- "))
          .mkString("\n")
