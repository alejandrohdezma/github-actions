...
trait Dsl
  ...
  /** Creates a GitHub Action workflow file definition that can be passed to the plugin's `workflows` setting.
    ...
    */
  def workflowFile(name: FileName)(workflow: Workflow): WorkflowFile =
    WorkflowFile(name, workflow)

  /** Entrypoint for creating workflows. Contains methods for providing all the different aspects of a GitHub Actions
    * workflow.
    *
    * @example {{{ workflow .name("Scala CI") .on(push.branches(matching("main"))) .on(pullRequest.branches(matching("main")))
    *   .job("build") { _.runsOn(`ubuntu-latest`) .steps( uses("actions/checkout@v2"), uses("actions/setup-java@v2")
    *   .name("Set up JDK 11") .withInput("java-version", "11") .withInput("distribution", "adopt"), run("sbt test")
    *   .name("Run tests") ) } }}}
    */
  lazy val workflow =
    Workflow.Builder1() /* scalafix:ok DisableSyntax.valInAbstract */
