...
object Runner {
  ...
  /** Allows converting a [[Runner]] value into [[yaml.Yaml yaml]]. */
  implicit val RunnerEncoder: Encoder[Runner] = {
    case h: HostedRunner   => h.value.asYaml
    case e: FromExpression => e.expression.asYaml
    case r: SelfHosted =>
      (List("self-hosted".asYaml, r.machine.asYaml, r.architecture.asYaml) ++
        r.extraLabels.map(_.asYaml)).filterNot(_.isNull()).asYaml
  }
